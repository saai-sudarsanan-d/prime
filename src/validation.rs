...
pub fn validate_deadline(deadline: &str) -> Result<&str, Error>{
    ...
    if dl_re.is_match(deadline) {
        ...
        Err(Error::new(ErrorKind::Other,"Please provide a proper deadline (--help for more details)"))
    }
}
...
pub fn validate_priority(priority: u8) -> Result<u8, Error>{
    if priority < 4 {
        ...
        Err(Error::new(ErrorKind::Other,"Priority must be greater than equal to 0 and less than 4!"))
    }
}